...
int Server::be_ready_for_connection()
{
   ...
   if(fd==-1){
    ...
    if (fcntl(fd, F_SETFL, O_NONBLOCK) == -1) //-> set the socket option (O_NONBLOCK) for non-blocking socket
        throw(runtime_error("faild to set option (O_NONBLOCK) on socket"));
    ...
    cout<<"server"<<fd_Server<<" connected and ready for receiving data"<<endl;
    cout<<"Waiting to accept conection"<<endl;
    while(1)
    {
        if(poll(&fds[0],fds.size(),-1) ==-1)
            throw (runtime_error("our pool function failed"));
        for(size_t i = 0;i < fds.size();i++)
        {
            if(fds[i].revents & POLLIN)
            {
                if(fds[i].fd == fd_Server)
                {
                    ...
                    cout<<"reicinvind a new data from a registred client";
                    //ReceiveNewData();
                }
            }
        }
